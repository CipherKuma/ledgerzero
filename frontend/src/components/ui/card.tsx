...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "cn-font-heading min-w-0 [overflow-wrap:anywhere] text-base leading-snug font-medium group-data-[size=sm]/card:text-sm",
        className
      )}
      {...props}
    />
...
function CardContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("min-w-0 px-(--card-spacing) [&.grid]:grid-cols-[minmax(0,1fr)]", className)}
      {...props}
    />
  )
}
